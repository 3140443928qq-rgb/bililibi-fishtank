  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-50">
      ...
      <div className="absolute top-4 left-4 pointer-events-auto flex flex-col gap-2">
        <div className="bg-black/60 backdrop-blur-md p-4 rounded-3xl border border-white/10 shadow-2xl flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-purple-500/20 px-3 py-1.5 rounded-full border border-purple-500/30">
            ...
            <span className="text-purple-400 font-black font-mono">{gameState.mutantPoints}</span>
          </div>
        ...
        <div className="flex gap-2">
          <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
            <Droplets className={gameState.waterQuality < 30 ? "text-red-500 animate-pulse" : "text-blue-400"} size={16} />
            <div className="w-24 h-2 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-blue-500 transition-all" style={{ width: `${gameState.waterQuality}%` }} />
            </div>
            <button 
              onClick={() => setGameState(prev => ({ ...prev, waterQuality: 100, coins: Math.max(0, prev.coins - 10) }))}
              className="text-[10px] bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded-lg font-bold"
            >
              清洗 (10金)
            </button>
          </div>
          <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-2 text-white font-mono text-sm">
            <Clock size={16} className="text-zinc-400" />
            {Math.floor(gameState.timeOfDay).toString().padStart(2, '0')}:00
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-4 right-4 pointer-events-auto flex flex-col gap-3">
        ...
        <button onClick={() => setShowShop(true)} className="p-4 bg-green-600 hover:bg-green-500 text-white rounded-2xl shadow-lg transition-all active:scale-95 flex items-center gap-2">
          <ShoppingBag size={20} />
          <span className="font-bold">商店</span>
        </button>
        <button onClick={() => setShowExtraction(true)} className="p-4 bg-purple-600 hover:bg-purple-500 text-white rounded-2xl shadow-lg transition-all active:scale-95 flex items-center gap-2">
          <Zap size={20} />
          <span className="font-bold">突变榨取</span>
        </button>
        <button onClick={() => setShowMaintenance(true)} className="p-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl shadow-lg transition-all active:scale-95 flex items-center gap-2">
          <Settings size={20} />
          <span className="font-bold">鱼缸升级</span>
        </button>
      </div>
          ...
          >
            <div className="bg-zinc-900 border-2 border-yellow-500 p-8 rounded-3xl w-full max-w-2xl shadow-2xl">
              ...
              <div className="flex gap-2 mb-6">
                {(['fish', 'deco', 'special'] as TabType[]).map(tab => (
                  <button 
                    key={tab}
                  ...
                  >
                    {tab === 'fish' ? '普通鱼类' : tab === 'deco' ? '装饰摆件' : '突变生物'}
                  </button>
                ))}
              </div>

              <div className="grid grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {Object.entries(currentTab === 'fish' ? FISH_CONFIG : currentTab === 'deco' ? DECO_CONFIG : SPECIAL_FISH).map(([name, config]) => {
                  const fishConfig = config as FishConfig;
                  const decoConfig = config as DecoConfig;
                  const isSpecial = currentTab === 'special';
                  const price = isSpecial ? fishConfig.mPrice : (currentTab === 'fish' ? fishConfig.price : decoConfig.price);
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-black text-white">
                    {selectedItem.type === 'fish' ? selectedItem.obj.type : selectedItem.obj.type}
                  </h3>
                  {selectedItem.type === 'fish' && (
                    <div className="text-yellow-500 text-sm font-bold">第 {selectedItem.obj.generation} 代</div>
                  )}
                </div>
